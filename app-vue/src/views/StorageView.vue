...
<style scoped>
.storage-page { max-width: 900px; margin: 28px auto; padding: 20px; font-family: Inter, sans-serif; color: #0b1220; }
.header { margin-bottom: 30px; }
.header h1 { margin: 0; font-size: 26px; letter-spacing: -0.01em; }
.lead { margin: 6px 0 0; color: #6b7280; font-size: 14px; }
.upload-card { background: #fff; padding: 30px; border-radius: 12px; box-shadow: 0 8px 24px rgba(2,6,23,0.06); display: flex; flex-direction: column; gap: 20px; align-items: center; }
.drop-zone { width: 100%; border: 2px dashed #e5e7eb; border-radius: 12px; padding: 40px 20px; text-align: center; position: relative; background: #fafafa; transition: all 0.2s ease; cursor: pointer; }
.drop-zone:hover { background: #f3f4f6; border-color: #d1d5db; }
.drop-zone.has-file { border-color: #10b981; background: #ecfdf5; }
.icon-cloud { font-size: 40px; margin-bottom: 12px; opacity: 0.8; }
.drop-zone h3 { margin: 0; font-size: 16px; font-weight: 600; color: #374151; }
.instructions { margin: 6px 0 0; color: #9ca3af; font-size: 13px; }
.file-name { margin: 10px 0 0; color: #059669; font-weight: 500; font-size: 14px; }
.file-input { position: absolute; top: 0; left: 0; width: 100%; height: 100%; opacity: 0; cursor: pointer; }
.btn-primary { background: linear-gradient(90deg, #1f2937, #374151); color: #fff; border: none; padding: 12px 28px; border-radius: 10px; cursor: pointer; font-weight: 600; font-size: 15px; box-shadow: 0 4px 12px rgba(31,41,55,0.2); transition: all 0.2s; display: flex; align-items: center; gap: 10px; }
.btn-primary:not(:disabled):hover { background: linear-gradient(90deg, #111827, #1f2937); transform: translateY(-2px); }
.btn-primary:disabled { background: #d1d5db; box-shadow: none; cursor: not-allowed; color: #9ca3af; }
.spinner { width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-top-color: transparent; border-radius: 50%; animation: spin 0.8s linear infinite; }
...
.badge { background: #e5e7eb; color: #374151; font-size: 12px; padding: 4px 10px; border-radius: 999px; font-weight: 600; }
.photos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; }
.photo-item { background: #fff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 12px rgba(2,6,23,0.04); border: 1px solid #f3f4f6; transition: transform 0.15s; }
.photo-item:hover { transform: translateY(-4px); box-shadow: 0 8px 16px rgba(2,6,23,0.08); }
.photo-preview { height: 150px; background: #f3f4f6; display: flex; align-items: center; justify-content: center; overflow: hidden; }
.photo-img { width: 100%; height: 100%; object-fit: cover; }
.photo-info { padding: 12px; }
.photo-url { font-size: 12px; color: #059669; margin: 0 0 6px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; }
.photo-id { margin: 0; font-size: 11px; color: #6b7280; font-weight: 500; }
.loading-state, .empty-state { text-align: center; padding: 40px; color: #6b7280; background: #f9fafb; border-radius: 12px; border: 1px dashed #e5e7eb; }
</style>